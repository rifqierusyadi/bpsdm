...
class Pengelola extends CI_Controller {
	...
	public $folder = 'referensi/pengelola/';
	
	public function __construct()
	{
		parent::__construct();
		$this->load->model('pengelola_m', 'data');
		signin();
		admin();
	}
	...
	{
		$data['head'] 		= 'Tambah Referensi Pengelola';
	...
	public function updated($id=null)
	{
		...
		$data['head'] 		= 'Ubah Referensi Pengelola';
	...
	public function ajax_list()
    {
        ...
        foreach ($record as $row) {
			...
			$col[] = $row->pengelola;
            $col[] = $row->instansi;
            $col[] = $row->api ? $row->api : 'Tidak Ada';
            //add html for action
            $col[] = '<a class="btn btn-xs btn-flat btn-warning" onclick="edit_data();" href="'.site_url('referensi/pengelola/updated/'.$row->id).'" data-toggle="tooltip" title="Edit"><i class="glyphicon glyphicon-pencil"></i></a>
	...
	public function ajax_save()
    {
        ...
        if($this->validation()){
			...
			helper_log("add", "Menambah Referensi Pengelola");
        }
    }
    ...
    public function ajax_update($id=null)
    {
        ...
        if($this->validation($id)){
			...
			helper_log("edit", "Merubah Referensi Pengelola");
        }
    }
    ...
    {
        $this->data->delete($id);
		helper_log("trash", "Menghapus Referensi Pengelola");
	...
	private function validation($id=null)
    {
		...
		if(!isset($id)){
			...
			$this->form_validation->set_rules("pengelola", "Pengelola", "trim|required");
		}else{
			$this->form_validation->set_rules("instansi", "Instansi Kerja", "trim|required");
			$this->form_validation->set_rules("pengelola", "Pengelola", "trim|required");
		}
	...
	public function get_pengelola(){
        ...
        if(!empty($parent)){
            ...
            echo form_dropdown('parent', array(''=>'Pilih Pengelola Induk'), '', "class='form-control select2' name='parent' id='parent'");
        }
    }
